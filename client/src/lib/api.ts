...
async function fetchWithAuth(
  ...
  if (response.status === 401) {
    ...
    // Don't redirect if already on login/signup/splash pages
    if (typeof window !== "undefined") {
      const path = window.location.pathname;
      const noRedirectPaths = ["/login", "/signup", "/splash", "/admin"];
...
export const businessApi = {
  ...
  async getMyBusiness(): Promise<Business | null> {
    try {
      ...
      if (response && 'business' in response && response.business === null) {
        return null;
      }
...
export const splashApi = {
  // Get splash page data
  async getSplashData(businessId: string): Promise<{
    business: {
      id: string;
      name: string;
      location?: string;

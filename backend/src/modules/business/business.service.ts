...
import { TreeProfile, TreeProfileDocument } from './schemas/tree-profile.schema';
import { WifiProfile, WifiProfileDocument, Ad } from './schemas/wifi-profile.schema';
...
import { CreateBusinessDto, UpdateBusinessDto, UpdateTreeProfileDto, UpdateWifiProfileDto, DashboardStatsDto } from './dto/business.dto';
...
export class BusinessService {
    ...
    constructor(
        ...
        @InjectModel(TreeProfile.name) private treeProfileModel: Model<TreeProfileDocument>,
    ...
    async create(ownerId: string, dto: CreateBusinessDto): Promise<BusinessDocument> {
        if (dto.username) {
    ...
    async findTreeProfile(businessId: string): Promise<TreeProfileDocument | null> {
        return this.treeProfileModel.findOne({ businessId: new Types.ObjectId(businessId) });
    }

    async findWifiProfile(businessId: string): Promise<WifiProfileDocument | null> {
        return this.wifiProfileModel.findOne({ businessId: new Types.ObjectId(businessId) });
    }

    async findOrCreateTreeProfile(businessId: string): Promise<TreeProfileDocument> {
        let profile = await this.treeProfileModel.findOne({ businessId: new Types.ObjectId(businessId) });
        if (!profile) {
            profile = await this.treeProfileModel.create({ businessId: new Types.ObjectId(businessId) });
        }
        return profile;
    }

    async findOrCreateWifiProfile(businessId: string): Promise<WifiProfileDocument> {
    ...
    async updateWifiProfile(
        ...
        if (dto.ads) {
            ...
            if (keysToDelete.length > 0) {
                this.logger.log(`Deleting ${keysToDelete.length} orphaned S3 images for business ${businessId}`);
                Promise.allSettled(
    ...
    async getDashboardStats(
        ...
        const totalAdViews = ads.reduce((sum: number, ad: Ad) => sum + (ad.views || 0), 0);
        const totalAdClicks = ads.reduce((sum: number, ad: Ad) => sum + (ad.clicks || 0), 0);

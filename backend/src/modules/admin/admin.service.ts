...
export class AdminService {
     ...
     */
    async requestOtp(dto: AdminRequestOtpDto, ipAddress?: string): Promise<OtpRequestResponseDto> {
        const email = dto.email.toLowerCase();

        // Check if email is in admin whitelist
        if (!this.isAdminEmail(email)) {
            this.logger.warn(`Unauthorized admin login attempt: ${email} from ${ipAddress}`);
        ...
        if (admin.otpExpiry && admin.otpExpiry > now) {
            const timeElapsed = now.getTime() - (admin.otpExpiry.getTime() - this.OTP_EXPIRY_MS);
    ...
    async verifyOtp(
        ...
        if (!admin) {
            throw new UnauthorizedException('Please request a verification code first');
        }
        ...
        // Log the login
        await this.logAccess(admin._id, admin.email, 'login', undefined, ipAddress, userAgent);
    ...
    async activateBusiness(
        ...
        this.logger.log(`Business activated: ${business.businessName} by admin: ${adminEmail}`);
    ...
    async rejectBusiness(
        ...
        this.logger.log(`Business rejected: ${business.businessName} by admin: ${adminEmail}`);
    ...
    async suspendBusiness(
        ...
        this.logger.log(`Business suspended: ${business.businessName} by admin: ${adminEmail}`);
    ...
    async getBusinessDetails(businessId: string): Promise<any> {
        ...
        const topPosts = ads.map((ad: any) => {
            ...
            const score = (views * 1) + (clicks * 5) + (likes * 3) + (shares * 10) + (expands * 2);
    ...
    async getAdInteractionDetails(adId: string): Promise<any[]> {
        const logs = await this.analyticsModel.find({ adId: new Types.ObjectId(adId) })
            .sort({ timestamp: -1 })
            .limit(50)

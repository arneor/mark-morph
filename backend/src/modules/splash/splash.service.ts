  ...
  async authenticateWithGoogle(
    ...
    if (business.status !== "active") {
      throw new BadRequestException(
        "This WiFi network is currently unavailable",
      );
    }
    ...
    this.logger.log(
      `Google OAuth successful for ${email} (${isNewUser ? "new" : "returning"} user) at business ${businessId}`,
    );

    // Link anonymous session analytics to this user
    if (sessionId) {
      this.analyticsService.linkSessionToUser(
        sessionId,
        wifiUser._id.toString(),
        email,
      );
    }
      ...
      if (wifiUser.otpWindowStart && wifiUser.otpWindowStart > oneHourAgo) {
        if (wifiUser.otpRequestCount >= this.MAX_OTP_REQUESTS_PER_HOUR) {
          const resetTime = new Date(
            wifiUser.otpWindowStart.getTime() + 60 * 60 * 1000,
          );

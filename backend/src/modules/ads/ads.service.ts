...
export class AdsService {
   ...
   */
  private async findOrCreateWifiProfile(
    businessId: string,
  ...
  async getAdsForBusiness(businessId: string, ownerId: string): Promise<Ad[]> {
    ...
    if (business.ownerId.toString() !== ownerId) {
      throw new ForbiddenException(
        "You do not have permission to view these ads",
      );
    }
  ...
  async createAd(
    ...
    if (business.ownerId.toString() !== ownerId) {
      throw new ForbiddenException(
        "You do not have permission to add ads to this business",
      );
    }
  ...
  async updateAd(
    ...
    const wifiProfile = await this.findOrCreateWifiProfile(businessId);
    const adIndex = wifiProfile.ads.findIndex(
  ...
  private async saveUploadedFile(
    ...
    if (videoExts.includes(ext)) {
      ...
      throw new BadRequestException(
        "Invalid file type. Allowed: jpg, jpeg, png, gif, webp, mp4, webm, mov, avi",
      );
    }
